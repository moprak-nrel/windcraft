...
class SolverTestCase(unittest.TestCase):
    ...
    def setUp(self):
        ...
        self.x, _ = np.meshgrid(
            np.linspace(0 + self.solver.dx,
                        self.solver.width - self.solver.dx,
                        self.solver.n[0] - 1),
            np.linspace(0 + 0.5 * self.solver.dx,
        ...
        np.random.seed(42)
        self.solver.p = 0.001 * np.random.rand(self.solver.n[0],
                                               self.solver.n[1])
    ...
    def test_reset(self):
        ...
        npt.assert_almost_equal(self.solver.v, 0.)
        npt.assert_almost_equal(self.solver.p, 0.)

    def test_apply_velocity_bc(self):
        """Is the velocity bc function correct?"""
        self.solver.apply_velocity_bc()
        npt.assert_almost_equal(np.linalg.norm(self.solver.ubc),
                                89.37910769214193)
        npt.assert_almost_equal(np.linalg.norm(self.solver.vbc),
                                81.86848953639407)
    ...
    def test_advection(self):
        ...
        npt.assert_almost_equal(np.linalg.norm(self.solver.u),
                                79.35577232785283)
        npt.assert_almost_equal(np.linalg.norm(self.solver.v),
                                81.62214012672611)
